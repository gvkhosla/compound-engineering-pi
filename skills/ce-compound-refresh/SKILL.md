...
- Ask questions **one at a time** — use the platform's blocking question tool when available (`ask_user_question` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini). Otherwise, present numbered options in plain text and wait for the user's reply before continuing
...
Always present choices using the platform's blocking question tool when available (`ask_user_question` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini). Otherwise, present numbered options in plain text and wait for the user's reply before proceeding.
