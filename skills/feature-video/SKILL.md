...
If no PR exists for the current branch, ask the user how to proceed. **Use the platform's blocking question tool** (`ask_user_question` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini):
...
**Use the platform's blocking question tool when available** (`ask_user_question` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini). Otherwise, present numbered options and wait for the user's reply before proceeding:
...
The user must log in manually in the browser window (handles 2FA, SSO, OAuth -- any login method). **Use the platform's blocking question tool** (`ask_user_question` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini). Otherwise, present the message and wait for the user's reply before proceeding:
