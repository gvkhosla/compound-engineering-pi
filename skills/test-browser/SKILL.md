...
Ask the user whether to run headed or headless (using the platform's question tool — e.g., `ask_user_question` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini — or present options and wait for a reply):
